...
app.get("/widget", (req, res) => {
  try {
    ...
    res.setHeader(
      "Cache-Control",
      `public, max-age=${process.env.WIDGET_CACHE_TTL || 300}`
    );
...
app.get("/campaigns/:campaignName", async (req, res) => {
  try {
    ...
    const campaign = {
      ...
      imageUrl:
        recommendation.products && recommendation.products[0]
          ? recommendation.products[0].image
          : null,
      ctaText: "Shop Now",
    ...
    console.log(
      `Campaign found: ${campaign.campaignName}, Match: ${campaign.matchReason}`
    );
